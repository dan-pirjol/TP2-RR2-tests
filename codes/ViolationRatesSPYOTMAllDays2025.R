...
source("computeViolationsAmericanOTMStrikes.R")

# load SPX options data for 2025
#my_data <- read_parquet("mktdata/spy_options_2025.parquet")
#allSPXData <- read.csv(file = 'mktdata/SPX-2025.csv')
#head(allSPXData)

#closeSPXData <- read.csv(file = 'mktdata/SPXprices2025.csv')
#head(closeSPXData)

#priceSPX <- closeSPXData$GSPC.Close
#length(priceSPX)

# load SPY options data for 2025
allSPYData <- read_parquet("mktdata/spy_options_2025.parquet")
head(allSPYData)

closeSPYData <- read.csv(file = 'mktdata/SPYprices2025.csv')
head(closeSPYData)

priceSPY <- closeSPYData$SPY.Close
length(priceSPY)
...
#      print(j)
      tij <- testTP2CallsAmerOTM(dataC1, dataC2, S0, S0, 0, 1.0, 1.1)
...
#      print(j)
      tij <- testRR2PutsAmerOTM(dataP1, dataP2, S0, S0, 0, 1.0, 1.1)
...
v.rates.all <- data.frame("day"=index.day, "vio.C"=bc.day,"no.C"=qc.day,"vr.c"=vrc.day,"vio.P"=bp.day,"no.P"=qp.day, 
                          "vr.p"=vrp.day,"vol.c"=volume.c.day, "vol.p"=volume.p.day)
head(v.rates.all)
tail(v.rates.all)


write.csv(v.rates.all, "VRatesSPY165days2025OTM.csv")
Fwd[1]
...
vrates.all.2025 <- read.csv(file = 'results/VRates165days2025.csv')
head(vrates.all.2025)

vrc.all <- vrates.all.2025$vr.c
vrp.all <- vrates.all.2025$vr.p

plot(1:n.asof, 100*vrc.day, type = "p", pch=20, col="blue", xaxt = "none", xlab = "date",
     ylim=c(0,6),ylab="Violation Ratios [%]",main="SPY TP2 violation ratios 2025 - OTM (solid), all strikes (dashed)")
lines(1:n.asof, 100*vrc.day, type = "l", col="blue")
lines(1:n.asof, 100*vrc.all, lty=2, type = "l", col="blue")
lines(1:n.asof, 100*vrp.day, type = "l", col="red")
lines(1:n.asof, 100*vrp.day, type = "p", pch=20, col="red")
lines(1:n.asof, 100*vrp.all, lty=2, type = "l", pch=20, col="red")
...
plot(1:n.asof, 100*vrp.day, type = "p", pch=20, col="red", xaxt = "none", xlab = "date",
     ylim=c(0,10),ylab="Violation Ratios [%]",main="SPY RR2 violation ratios 2025 - OTM (solid), all strikes (dashed)")
lines(1:n.asof, 100*vrp.day, type = "l", col="red")
lines(1:n.asof, 100*vrp.day, type = "p", pch=20, col="red")
lines(1:n.asof, 100*vrp.all, lty=2, type = "l", pch=20, col="red")
