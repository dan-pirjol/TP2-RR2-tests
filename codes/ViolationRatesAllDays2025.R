...
allStrikesC <- data.frame("T"=c(),"CallsK"=c())
allStrikesP <- data.frame("T"=c(),"PutsK"=c())
...
#plot(Tdays,n.KP, type="s", pch=20,col="red", main="SPY: Traded strikes vs maturity. Calls(blue), puts(red) ", 
#     ylim=c(0,200), xlim=c(0,1100))
#lines(Tdays,n.KC, type="s", pch=20,col="blue")

Cstrikes.day <- data.frame("T"=Tdays,"CallsK"=n.KC)
Pstrikes.day <- data.frame("T"=Tdays,"PutsK"=n.KP)

allStrikesC <- rbind(allStrikesC,Cstrikes.day)
allStrikesP <- rbind(allStrikesP,Pstrikes.day)
...
# aggregate the number of traded strikes per days to maturity
head(allStrikesP)
tail(allStrikesP)

#StrikesC <- aggregate(allStrikesC, by=list(allStrikesC$T), FUN=sum)
#head(StrikesC)

StrikesC <- aggregate(allStrikesC$CallsK ~ allStrikesC$T, data=allStrikesC, FUN=sum)
StrikesP <- aggregate(allStrikesP$PutsK ~ allStrikesP$T, data=allStrikesP, FUN=sum)

head(StrikesP)
tail(StrikesP)

names(StrikesC)=c("T","CallsK")
names(StrikesP)=c("T","PutsK")

#length(StrikesC2$`allStrikesC$T`)
#aggregate(value_col ~ group_col, data = df, FUN = sum)


plot(StrikesC$T, StrikesC$CallsK/165, type="h", col="blue",ylim=c(-10,10),
     main="SPY Strikes/day traded vs T. Calls (blue), Puts (red)", xlab="T [days]", ylab="Strikes count")
lines(StrikesP$T,-StrikesP$PutsK/165, type="h", col="red")
abline(h=0,col="black")

plot(StrikesP$T,StrikesP$PutsK, type="h", col="red")
