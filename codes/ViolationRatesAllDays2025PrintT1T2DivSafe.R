...
vrp12.day <- c()
#----------------------
#loop through all AsOfDates 1:20 Jan and 61:81 Apr
#for(ias in 1:n.asof){
for(ias in 61:81){
...
# run only over 30 days maturity
#for (i in 2:n.exp){
#  for (j in 2:n.exp){
for (i in 2:imax){
  for (j in 2:imax){
...
write.csv(v.call.all, "VCallDataT1T2-safe-2025.csv")
write.csv(v.put.all, "VPutDataT1T2-safe-2025.csv")
Fwd[1]
...
plot(vcT1,vcT2,type="p",pch=20,col="grey",
     main="Calls DivSafe 2025. TP2 vr > 1%(blue),2%(red),5%(yellow)",
...
plot(vpT1,vpT2,type="p",pch=20,col="grey",
     main="Puts DivSafe 2025. RR2 vr > 1%(blue),2%(red),5%(yellow)",
     xlab="T1 [days]", ylab="T2 [days]",
     xlim=c(0,30), ylim=c(0,30))
